...
import { DriveToolSearchModal } from '@/components/drive/DriveToolSearchModal';
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' && selectedIds.length > 0) {
        ...
        e.preventDefault();
        const selected = selectedIds.length === 1 ? items.find((i) => i.id === selectedIds[0]) : null;
        openToolSearch(selected);
      }
    };
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-white dark:bg-zinc-900/60">
        ...
        {/* Mobile Pull-to-Refresh & Scrollable Items Feed */}
        <div className="flex-1 overflow-hidden relative z-10">
          <DrivePullToRefresh onRefresh={async () => { await loadItems(); }}>
            <div className="p-4 sm:p-6 lg:p-8 pb-28 sm:pb-12 min-h-full">
        ...
        {/* Mobile Floating Action Button (Thumb Speed Dial) */}
        <DriveMobileFAB onOpenNewFolderModal={() => setNewFolderOpen(true)} />
      </main>
      ...
      {/* Universal FileCraft Tool Search & Launch Modal */}
      <DriveToolSearchModal />
    </div>
  );
}
