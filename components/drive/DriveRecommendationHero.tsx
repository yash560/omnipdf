...
export function DriveRecommendationHero({
  ...
  return (
    <section className="mb-6 rounded-2xl bg-gradient-to-b from-white to-zinc-50/80 dark:from-zinc-900 dark:to-zinc-950/80 border border-zinc-200/80 dark:border-zinc-800/80 shadow-xs overflow-hidden transition-all duration-200">
      {/* Top Header Bar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-3.5 sm:px-5 py-2.5 sm:py-3 border-b border-zinc-100 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/40 gap-2.5 sm:gap-3">
        <div className="flex items-center justify-between w-full sm:w-auto">
          ...
          <button
            ...
            className="sm:hidden p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
        ...
        <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar" style={{ touchAction: 'pan-x' }}>
          {/* Stream Navigation Tabs */}
          <div className="flex items-center bg-zinc-200/60 dark:bg-zinc-800/60 p-0.5 rounded-xl text-xs font-bold overflow-x-auto no-scrollbar shrink-0">
            <button
              type="button"
              onClick={() => {
              ...
              className={`px-2.5 sm:px-3 py-1 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shrink-0 ${
          ...
          <button
            type="button"
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="hidden sm:flex p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer shrink-0"
            title={isCollapsed ? 'Expand Recommendations' : 'Collapse Recommendations'}
          >
            {isCollapsed ? <ChevronDown className="w-4 h-4" /> : <X className="w-4 h-4" />}
          </button>
