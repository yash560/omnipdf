        ...
        <div className="w-80 sm:w-96 shrink-0 h-full flex flex-col bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 overflow-y-auto p-5 space-y-5 animate-drawer-left">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Tooltip content="Open Quick Look Preview" shortcut="Space" side="top">
              <button
                type="button"
                onClick={() => openPreview(selectedInspectorItem)}
                className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-zinc-900 hover:bg-black dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 font-bold text-xs shadow-xs transition-colors cursor-pointer"
              >
                <Eye className="w-3.5 h-3.5" />
                <span>Preview</span>
              </button>
            </Tooltip>
            <Tooltip content="Open file transformation & AI tools studio" side="top">
              <button
                type="button"
                onClick={() => openQuickTools(selectedInspectorItem)}
                className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-gradient-to-r from-rose-500 to-indigo-600 hover:from-rose-600 hover:to-indigo-700 text-white font-bold text-xs shadow-xs transition-all cursor-pointer"
              >
                <Wrench className="w-3.5 h-3.5" />
                <span>Quick Tools</span>
              </button>
            </Tooltip>
            <Tooltip content={selectedInspectorItem.isStarred ? 'Remove from Starred' : 'Add to Starred'} side="top">
              <button
                type="button"
                onClick={() => toggleStar(selectedInspectorItem.id)}
                className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 font-bold text-xs shadow-2xs transition-colors cursor-pointer"
              >
                <Star
                  className={`w-3.5 h-3.5 ${
            ...
            <Tooltip content="Share file or create secure expiring link" side="top">
              <button
                type="button"
          ...
          <div className="space-y-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center justify-between text-3xs font-extrabold uppercase tracking-wider text-zinc-400">
              ...
              <Tooltip content="Re-run OCR & AI entity extraction" side="left">
                <button
                  type="button"
            ...
            <div className="space-y-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
              ...
              <div className="space-y-1">
                {getFileCraftToolsForItem(selectedInspectorItem).map((tool) => (
                  <Tooltip key={tool.href} content={`Launch ${tool.label} with this document`} side="left">
                    <Link
